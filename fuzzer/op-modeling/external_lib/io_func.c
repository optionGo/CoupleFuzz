...
#include "./variable_handers.h"

#define VARIABLE_LABEL_OFFSET 20
#define MAX_VARIABLE_LABEL_COUNT 1 << (32 - VARIABLE_LABEL_OFFSET)
...
int hash_table_insert(HashTable *ht, void *key, dfsan_label value) {
  if (!ht || !key) return -1;  

  
  unsigned int hash = ptr_hash(key);
...
dfsan_label hash_table_find(HashTable *ht, const void *key) {
  if (!ht || !key ) return -1;
...
int hash_table_delete(HashTable *ht, const void *key) {
  if (!ht || !key) return -1;
...
size_t hash_table_count(HashTable *ht) {
  return ht ? ht->count : 0;
}


HashTable *addr_lb = NULL;
static int granularity = 1; // byte level
extern void __tag_set_fini();
extern void __variable_counter_fini();
// extern void __chunk_set_input_file_name(int );


static void cleanup_function(void) {
  printf("通过atexit调用清理函数！\n");
  __variable_counter_fini();
  __tag_set_fini();
}


__attribute__((constructor(0))) void __angora_track_init(void) {
  printf("动态库正在被加载！\n");
...
void __chunk_set_input_file_name(int fsize) {
...
void __variable_set_label(void *addr, int id) {
  
  if (!addr_lb) {
    addr_lb = hash_table_create(MAX_VARIABLE_LABEL_COUNT);
  }
  
  // printf("into __variable_set_label, addr = %p, id = %d\n", addr, id);
  dfsan_label lb = option_variable_tag_set_insert(id);
  printf("assign varibale label addr = %p, id = %d, label = %d\n", addr, id, lb >> 20);
  dfsan_set_label(lb, addr, 1);
  
  
  hash_table_insert(addr_lb, addr, lb);
}

__attribute__((visibility("default")))
void __variable_restore(void *addr) {
  dfsan_label lb = hash_table_find(addr_lb, addr);
  if (lb != -1) {
    dfsan_set_label(lb, addr, 1);
  }
}
...
__dfsw_fopen(const char *filename, const char *mode, dfsan_label fn_label,
  ...
  if (fd && IS_FUZZING_FILE(filename)) {
    ...
    printf("### fopen, add fuzzing flename: %s\n", filename);
...
__dfsw_fopen64(const char *filename, const char *mode, dfsan_label fn_label,
  ...
  if (IS_FUZZING_FILE(filename)) {
    DEBUG_PRINTF("### fopen, add fuzzing fd\n");
    printf("### fopen64, add fuzzing flename: %s\n", filename);
    setenv("CHUNK_CURRENT_INPUT_FILE", filename, 1);
    fseek(fd, 0, SEEK_END);
    int fsize = ftell(fd);
    fseek(fd, 0, SEEK_SET);
