...
#include "llvm/IR/CallSite.h"  // Removed in LLVM 14+
...
std::string getStructPointer(Instruction *I) {
  std::string structPointer = "";
  ...
  if (CallInst *callInst = dyn_cast<CallInst>(I)) {
    ...
    firstArg = storeInst->getPointerOperand();
  } else {
    return "";
  }
  ...
  auto peelBitCast = [](Value *V) -> Value * {
    while (true) {
  ...
  if (auto *CE = dyn_cast<ConstantExpr>(firstArg)) {
    if (CE->getOpcode() == Instruction::GetElementPtr) {
      ...
      std:: string structFirstLocalPointer = "";
      if (auto *GEP = dyn_cast<GetElementPtrInst>(TmpInst)) {
        if (StructType *ST = dyn_cast<StructType>(GEP->getSourceElementType())) {
          StringRef name = ST->getName();
          if (!name.empty()) {
            structPointer += name.str() + ".";
          }
        }
        if (GEP->getPointerOperand() && GEP->getPointerOperand()->hasName()) {
          StringRef name = GEP->getPointerOperand()->getName();
          if (!name.empty()) {
            structPointer += name.str();
            structFirstLocalPointer = name.str();
          }
        }
        for (auto idx = GEP->idx_begin(); idx != GEP->idx_end(); ++idx) {
          if (auto *CI = dyn_cast<ConstantInt>(*idx)) {
            structPointer += "[" + utostr(CI->getSExtValue()) + "]";
          } else {
            structPointer += "[?]";
          }
        }
      ...
      if (structPointer.find("[0][0]") != std::string::npos) {
        return structFirstLocalPointer;
      }
  ...
  if (auto *GEP = dyn_cast<GetElementPtrInst>(firstArg)) {
    std:: string structFirstLocalPointer = "";
    if (StructType *ST = dyn_cast<StructType>(GEP->getSourceElementType())) {
      StringRef name = ST->getName();
      if (!name.empty()) {
  ...
  if (auto *GV = dyn_cast<GlobalVariable>(firstArg)) {
  ...
  if (firstArg && firstArg->getType() && firstArg->getType()->isPointerTy()) {
    PointerType *ptrType = dyn_cast<PointerType>(firstArg->getType());
    if (ptrType && ptrType->getElementType()->isPointerTy()) {
      PointerType *innerPtrType = dyn_cast<PointerType>(ptrType->getElementType());
      if (innerPtrType && innerPtrType->getElementType()->isIntegerTy(8)) {
        
        if (firstArg->hasName()) {
          StringRef name = firstArg->getName();
          if (!name.empty()) {
            structPointer += name.str();
            return structPointer;
          }
...
class DataFlowSanitizer : public ModulePass {
  ...
  FunctionType *OptionSetLabelFnTy;
  ...
  FunctionCallee OptionSetLabelFn;
...
struct DFSanFunction {
  ...
  DenseSet<AllocaInst *> optionSetLabelMap;
  DenseSet<std::string> optionSetStructLabelMap;
...
bool DataFlowSanitizer::doInitialization(Module &M) {
  ...
  Type *OptionSetLabelArgs[2] = {Type::getInt8PtrTy(*Ctx), Int32Ty };
  OptionSetLabelFnTy = FunctionType::get(Type::getVoidTy(*Ctx),
                                        OptionSetLabelArgs, /*isVarArg=*/false);
...
bool DataFlowSanitizer::runOnModule(Module &M) {
...

  {
    AttributeList AL;
    AL = AL.addParamAttribute(M.getContext(), 0, Attribute::ZExt);
    OptionSetLabelFn = Mod->getOrInsertFunction("__variable_set_label", OptionSetLabelFnTy, AL);
  }
  DFSanNonzeroLabelFn =
  ...
  for (Function &i : M) {
    if (!i.isIntrinsic() &&
        ...
        // &i != OptionSetLabelFn.getCallee()->stripPointerCasts())
...
void DFSanFunction::storeShadow(Value *Addr, uint64_t Size, uint64_t Align,
  ...
  if (AllocaInst *AI = dyn_cast<AllocaInst>(Addr)) {
    ...
    // const auto j = optionSetLabelMap.find(AI);
    // if (j != optionSetLabelMap.end()) {
    //   return;
    // }
  }

  // std::string structPointer = getStructPointer(Pos);
  // if (structPointer != "" && 
  //       optionSetStructLabelMap.find(structPointer) != optionSetStructLabelMap.end()) {
  //     errs() << "DFSan: process store shadow for structPointer: " << structPointer << "\n";
  //     return;
  // }
...
void DFSanVisitor::visitAllocaInst(AllocaInst &I) {
  ...
  for (User *U : I.users()) {
    ...
    if (CallInst *CI = dyn_cast<CallInst>(U)) {
      ...
      if (callee && callee->getName().contains("__variable_set_label")) {
        StringRef name = I.getName();
        errs() << "Alloca " << (name.empty() ? "<unnamed>" : name) << " escapes via call to __variable_set_label\n";
        DFSF.optionSetLabelMap.insert(&I);
        return;
      }
    }
...
void DFSanVisitor::visitCallSite(CallSite CS) {
  ...
  // if (F && F->hasName() && F->getName().contains("__variable_set_label") && !F->isDeclaration()) {
  //   Instruction *Inst = CS.getInstruction();
  //   if (!Inst) {
  //       errs() << "Warning: Instruction is null in visitCallSite\n";
  //       return;
  //   }
  
  //   if (Inst->getParent() == nullptr) {
  //       errs() << "Warning: Instruction has no parent\n";
  //       return;
  //   }
  //   std::string structPointer = getStructPointer(CS.getInstruction());
  
  //   errs() << "DFSan: process call site for structPointer: " << structPointer << "\n";
  //   if (structPointer != "") {
  //     DFSF.optionSetStructLabelMap.insert(structPointer);
  //   }
  // }
  ...
  if (i != DFSF.DFS.UnwrappedFnMap.end()) {
    ...
    case DataFlowSanitizer::WK_Custom:
      ...
      if (CallInst *CI = dyn_cast<CallInst>(CS.getInstruction())) {
        ...
        // errs() << "DFSan: process call site for function: " << F->getName() << "\n";
        // errs() << "DFSan: num params: " << FT->getNumParams() << "\n";
