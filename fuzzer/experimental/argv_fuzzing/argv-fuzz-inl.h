...
#define AFL_INIT_ARGV() do { argv = afl_init_argv(&argc, argv, argc); } while (0)

#define AFL_INIT_SET0(_p) do { \
    argv = afl_init_argv(&argc, argv, argc); \
    argv[0] = (_p); \
    if (!argc) argc = 1; \
  } while (0)
...
static char** afl_init_argv(int* argc, char **argv, int argc_origin) {

  static char  in_buf[MAX_CMDLINE_LEN];
  static char* receive[MAX_CMDLINE_PAR];
  ...
  int new_argc = 1;
  for (i = 1; i < argc_origin; i++) {
    if (argv[i] && !strcmp(argv[i], placeholder)) {
      for (int j = 1; j < rc ; j++) {
        ret[new_argc++] = receive[j];
      }
    } else {
      ret[new_argc++] = argv[i];
    }
